...
import { DeleteAdminDTO } from './dto/deleteAdmin.dto';
  ...
  export class AdminController {
    ...
    async deleteAdmin(@Body() adminData: DeleteAdminDTO) {
      const deletedAdmin =
        await this.adminService.deleteAdmin(adminData);
      return deletedAdmin;
    }
    ...
    async createVerifiedAdmin(@User() user:any, @Body() adminData: CreateAdminDTO) {
      try {
        const newAdmin =
          await this.adminService.createVerifiedAdmin(adminData);
        return newAdmin;
      } catch (error) {
        // Handle errors or exceptions as per your application's needs
        throw error;
    ...
    async verifyAdmin(@Body() adminData: VerifyAdminDTO) {
      try {
        const newAdmin =
          await this.adminService.verifyAdmin(adminData);
        return newAdmin;
      } catch (error) {
        // Handle errors or exceptions as per your application's needs
        throw error;
